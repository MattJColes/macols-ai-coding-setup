...
- Replace the old unsloth entry under this provider while preserving unrelated models, providers and settings. Migrate selections referencing that old provider/model pair. Select Swift with medium reasoning for omp's default and vision roles and its plan role with `:xhigh` reasoning, with text and image inputs enabled; retain explicit role overrides.
