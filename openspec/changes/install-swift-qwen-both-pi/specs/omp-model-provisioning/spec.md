...
It SHALL replace `unsloth/Qwen3.8-27B-NVFP4` only under `vllm-lan` and migrate selections of that provider/model pair. Other providers, models and selections SHALL survive. Repeated installs SHALL produce exactly one Swift entry. Malformed user configuration SHALL fail without being overwritten.
...
- **THEN** both agents receive the Swift provider registration
...
- **THEN** both agents receive the LAN provider and the existing default is preserved
