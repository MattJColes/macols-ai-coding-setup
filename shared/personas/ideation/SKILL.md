...
- For the shortlist only, add one line each on the riskiest assumption and the
  cheapest way to test it (hand to **product-manager** for a proper
...
- **product-manager** — takes the shortlist forward: JTBD framing, riskiest-
  assumption tests, sizing. Ideation ends where the hypothesis template begins.
- **architecture-expert** — feasibility pass on technical big bets *after*
  convergence, never during divergence.
- **ui-ux-designer** — sketching the experience for shortlisted product ideas.
- **interrogate-me** — when the brainstorm keeps stalling, the framing is
