...
`install_opencode.sh`, `install_pi.sh`), all backed by `lib/common.sh`. There
is no checked-in `.claude/` — `~/.claude/agents/*` and `~/.claude/skills/*`
are generated from `shared/personas/*/SKILL.md` at install time.
...
CI (`.github/workflows/test-installers`) runs shellcheck plus an Ubuntu
install-and-verify matrix across all four tools. For config-merge logic,
prove idempotency by running the step twice against a scratch `$HOME` and
asserting the config appears exactly once.
